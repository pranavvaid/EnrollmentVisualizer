...
def retrieveDepartmentCourses(departmentName):
    ...
    for course in entireCourseData['xml']['courses']['course']:
        importantData = {key: course[key] for key in ['title', 'description', 'unitsMin', 'unitsMax', 'subject', 'code']}
        importantData['sections'] = []
        if course['sections']:
            if type(course['sections']['section']) == type([]):
                for section in course['sections']['section']:
                    currSectionData = {key: section[key] for key in ('sectionNumber', 'term', 'component', 'numEnrolled', 'maxEnrolled', 'currentClassSize', 'maxClassSize', 'currentWaitlistSize', 'maxWaitlistSize')}
                    importantData['sections'].append(currSectionData);
            else:
                currSectionData = {key: course['sections']['section'][key] for key in ('sectionNumber', 'term', 'component', 'numEnrolled', 'maxEnrolled', 'currentClassSize', 'maxClassSize', 'currentWaitlistSize', 'maxWaitlistSize')}
                importantData['sections'].append(currSectionData);
...
def createCourseMap(allCourses, allDepartments):
    ...
    for course in allCourses:
        ...
        if currentCourse is None:
            currentCourse = StanfordClass(course['title'], course['description'], course['unitsMin'], course['unitsMax'], course['subject'] + " " + course['code'], course['sections'], [], [])
        else:
            ...
            currentCourse.sections = course['sections']
...
pickleFileName = "/pickles/stanfordclasslistAFTERANNOUNCEMENT" + datetime.now().strftime('%m_%d_%H_%M_%S') + ".pkl"
#'/pickles/stanfordclasslistAFTERANNOUNCEMENT4-1.pkl'
